...
test("transcript omits thinking, images, and recap entries while redacting tool data", () => {
  const bearerToken = ["very", "secret", "token"].join("-");
  const authorizationHeader = ["Authorization:", "Bearer", bearerToken].join(
    " ",
  );
  const apiKey = ["sk", "super", "secret", "value"].join("-");
  const resultToken = ["another", "secret"].join("-");
  const entries: SessionEntry[] = [
    ...
    entry("assistant", {
      ...
      content: [
        { type: "thinking", thinking: "hidden chain of thought" },
        {
          ...
          name: "bash",
          arguments: {
            command: `curl -H '${authorizationHeader}' https://example.test`,
            apiKey,
    ...
    entry("result", {
      ...
      toolCallId: "call-1",
      toolName: "bash",
      content: [{ type: "text", text: `token=${resultToken}\nfinished` }],
      isError: false,
      timestamp: 2,
    }),
    {
  ...
  assert.doesNotMatch(transcript, new RegExp(bearerToken));
  assert.doesNotMatch(transcript, new RegExp(resultToken));
